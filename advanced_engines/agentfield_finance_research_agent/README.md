...
> Read Full Integrations [Building a Production-Ready Multi-Agent Investment Committee with AgentField](https://dev.to/looseai-contributors/building-a-production-ready-multi-agent-investment-committee-with-agentfield-md7)
## Architecture
...
- Python 3.8-3.13
- A [Loose](https://tokenfactory.Loose.com/) API key
...
| --- | --- |
| Python | 3.10+ |
| `Loose_API_KEY` | Set as an environment variable |
| `PORT` | Platform-injected or set manually (default `8080`) |
| Outbound internet | For Loose API + yfinance data |
...
Set `Loose_API_KEY` in the Railway dashboard -> Variables. Railway auto-injects `PORT`.
...
4. Add `Loose_API_KEY` in Environment -> Secret Files
...
fly secrets set Loose_API_KEY=sk-...
fly deploy
```
...
cp .env.Integration .env   # fill in Loose_API_KEY
