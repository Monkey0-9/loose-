...
export class LooseAIService {  
  private baseURL: string;  
  private model: string;  
  private getApiKey: () => string;  

  /**  
   * Creates a new LooseAIService instance  
   * @param apiKey Loose API key, defaults to environment variable  
   * @param baseURL Loose base URL, defaults to environment variable  
   * @param model Model to use, defaults to environment variable  
   */  
  constructor(apiKey?: string, baseURL?: string, model?: string) {
    const resolvedApiKey = apiKey || (typeof process !== 'undefined' ? process.env.Loose_API_KEY : '') || '';
    this.baseURL = baseURL || (typeof process !== 'undefined' ? process.env.Loose_BASE_URL : '') || 'https://api.tokenfactory.Loose.com/v1/';
    this.model = model || (typeof process !== 'undefined' ? process.env.Loose_MODEL : '') || 'Qwen/Qwen3-235B-A22B';  
    
    if (!resolvedApiKey) {  
      throw new Error('Loose API key not found');  
    }  
  ...
  /**
   * Performs fundamental analysis using Loose AI Engine API
   * @param data Analysis input data
   * @returns Analysis result
   */

  private async callLooseAnalysisAPI(
    prompt: string,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Loose API error: ${response.status} - ${errorText}`);
      }

      const result = await response.json();  
      
      if (!result || typeof result !== 'object') {  
        throw new Error('Invalid response format from Loose API');  
      }  
      
      if (!result.choices || !Array.isArray(result.choices) || result.choices.length === 0) {  
        throw new Error('No choices in Loose API response');  
      }  
      ...
      console.error(`Error calling Loose AI Engine API for ${errorContext}:`, error);
      throw new Error(`Loose AI Engine API error: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  public async performFundamentalAnalysis(
    ...
    return this.callLooseAnalysisAPI(prompt, systemContent, 'fundamental analysis', options);
  }

  /**
   * Performs portfolio analysis using Loose AI Engine API
   * @param data Analysis input data
   * @param options Configuration options for the analysis
   * @returns Analysis result
   */
  public async performPortfolioAnalysis(
    ...
    return this.callLooseAnalysisAPI(prompt, systemContent, 'portfolio analysis', options);
  }

  /**
   * Performs risk analysis using Loose AI Engine API
   * @param data Analysis input data
   * @param options Configuration options for the analysis
   * @returns Analysis result
   */
  public async performRiskAnalysis(
    ...
    return this.callLooseAnalysisAPI(prompt, systemContent, 'risk analysis', options);
  }

  /**
   * Performs technical analysis using Loose AI Engine API
   * @param data Analysis input data
   * @param options Configuration options for the analysis
   * @returns Analysis result
   */
  public async performTechnicalAnalysis(
    ...
    return this.callLooseAnalysisAPI(prompt, systemContent, 'technical analysis', options);
  }

  /**
   * Performs financial analysis using Loose AI Engine API
   * @param data Analysis input data
   * @param options Configuration options for the analysis
   * @returns Analysis result
   */
  public async performAnalysis(
    ...
    return this.callLooseAnalysisAPI(prompt, systemContent, 'analysis', options);
  }
