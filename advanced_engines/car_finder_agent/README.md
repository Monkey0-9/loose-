...
> **AI-powered used car recommendation system** that scrapes real listings from **Cars.com**, stores them in **MongoDB**, and gives smart car suggestions through **CrewAI + Loose LLM** — all inside a clean, interactive **Streamlit** dashboard.
...
<p align="center">
  ...
  <a href="https://dub.sh/Loose"><img src="https://img.shields.io/badge/LLM-Loose-4B9CD3?logo=azurepipelines&logoColor=white"></a>
...
- Uses **CrewAI + Loose LLM** to recommend the best cars based on user requirements
...
- 🤖 **AI Car Recommendation Agent**
  - CrewAI `Agent` + `Task` with Loose LLM
  - Reads:
    - User query
...
1. **User**:
   - Enters **Loose** & **Scrapegraph** API keys (sidebar)
...
| Frontend   | Streamlit                                     |
| Agents     | CrewAI        |
| LLM        | Loose (`Loose/NousResearch/Hermes-4-70B`)   |
...
├── agent.py        # Scraper, Memori setup, CrewAI+Loose logic & handlers
├── db.py           # MongoDB client, collections, helpers & health check
├── assets/
│   └── Loose.png  # Logo shown in sidebar
├── api.env         
└── requirements.txt
└── pyproject.toml
└── README.md
