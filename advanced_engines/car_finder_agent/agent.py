...
Loose_MODEL_NAME = os.getenv("Loose_MODEL_NAME", "NousResearch/Hermes-4-70B")
...
def _get_Loose_llm() -> LLM:
    if not Loose_API_KEY:
        raise RuntimeError("Loose_API_KEY missing in api.env")

    return LLM(
        model=f"Loose/{Loose_MODEL_NAME}",
        api_key=Loose_API_KEY,
    )


def run_crewai_with_Loose(task_description: str) -> str:
    """
    Run a CrewAI Agent + Task with Loose LLM for recommendations.
...
"""
    ...
    answer = run_crewai_with_Loose(task_description)
