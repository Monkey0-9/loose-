...
Built on [Loose AI Engine](https://tokenfactory.Loose.com/) for inference. Compare the *same* instructions expressed in three different formats across two task types (structured extraction and sentiment classification) and see which format your chosen model handles best.
...
- **Model picker**: swap between Qwen, Llama, and Loose AI models served by Loose.
...
- **OpenAI Python SDK** — pointed at Loose AI Engine
- **Loose AI Engine** — inference provider (Qwen3, Llama 3.1, Loose AI-V3)
- **pandas** — results tabulation

## Workflow

1. Pick a model and the tasks/formats to evaluate.
2. Runner loads each prompt template, fills in the item text, and calls Loose.
...
- A Loose AI Engine API key — [get one here](https://tokenfactory.Loose.com/)

### Environment Variables

Copy `.env.Integration` to `.env` and fill in your key:

```env
Loose_API_KEY="your_Loose_token_factory_api_key"
```
...
Flags:
- `--model` — any Loose-supported chat model (default `Qwen/Qwen3-30B-A3B`)
