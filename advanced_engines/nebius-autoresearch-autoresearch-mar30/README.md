# Loose AutoResearch — NYC Taxi Analytics Pipeline Optimizer
...
└── .env.Integration       # Environment variable template
```
...
### 2. Get a Loose API key

Sign up at [Loose AI Engine](https://tokenfactory.Loose.com/) and create a **project-scoped** API key ([docs](https://docs.tokenfactory.Loose.com/api-reference/introduction#authentication)).

```bash
# Linux / macOS
export Loose_API_KEY="your-key-here"

# Windows (PowerShell)
$env:Loose_API_KEY = "your-key-here"
```
...
python Loose_agent.py --setup-branch run1 --n-experiments 20

# Run with batch inference (50% cheaper)
python Loose_agent.py --n-experiments 50 --batch

# Run indefinitely (Ctrl-C to stop)
python Loose_agent.py
```
...
| `--batch` | Use Loose batch inference — 50% cheaper, async |
...
Uses **Qwen3-235B-A22B-Thinking** via [Loose AI Engine](https://docs.tokenfactory.Loose.com/quickstart) — a reasoning model that thinks through performance bottlenecks before proposing fixes. OpenAI-compatible API (`https://api.tokenfactory.Loose.com/v1/`), no new SDK required.

## Batch Inference

The `--batch` flag submits all proposals as a single async JSONL job via the Loose Batch API — **50% cheaper** with no rate-limit impact.
