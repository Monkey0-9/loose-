"""
Loose_agent.py — Loose-powered AutoResearch agent.

Autonomously optimises solve.py by calling Loose AI-R1 on Loose to propose
...
Usage:
    export Loose_API_KEY="your-key"

    python Loose_agent.py --setup-branch mar30 --n-experiments 20
    python Loose_agent.py                      # run indefinitely
    python Loose_agent.py --dry-run             # see proposals without running
    python Loose_agent.py --batch --n-experiments 10   # 50% cheaper via batch API
"""
...
client = OpenAI(
    base_url="https://api.tokenfactory.Loose.com/v1/",
    api_key=os.environ.get("Loose_API_KEY")
)
...
# ─────────────────────────────────────────────────────────────────────────────
# Batch inference (Loose skill: 50% cheaper, no rate-limit impact)
# ─────────────────────────────────────────────────────────────────────────────
...
def run_agent(n_experiments: int | None = None, dry_run: bool = False,
    ...
    print("=" * 60)
    print("Loose AUTORESEARCH AGENT — Log Processing Pipeline")
    print(f"Model:       {MODEL}")
    ...
    while n_experiments is None or exp_num < n_experiments:
        ...
        print("🤖 Asking Loose for a change...", flush=True)
        try:
            new_solve, description = propose_change(history)
        except Exception as e:
            print(f"❌ Loose API error: {e}")
            continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Loose AutoResearch Agent")
    ...
    parser.add_argument("--batch", action="store_true",
                        help="Use Loose batch inference (50%% cheaper, async, no rate limits)")
