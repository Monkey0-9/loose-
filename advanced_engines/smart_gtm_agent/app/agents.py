...
if not SMARTCRAWLER_KEY or not Loose_KEY:
    raise EnvironmentError(
        "SMARTCRAWLER_API_KEY and Loose_API_KEY must be set in api.env"
    )

sg_client = Client(api_key=SMARTCRAWLER_KEY)
llm = ChatLoose(model="NousResearch/Hermes-4-70B", api_key=SecretStr(Loose_KEY))
