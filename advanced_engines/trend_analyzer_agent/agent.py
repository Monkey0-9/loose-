...
load_dotenv()
api_base = os.getenv("Loose_API_BASE")
...
def firecrawl_scrape_Loose(_: str) -> dict:
    firecrawl = FirecrawlApp(api_key=os.getenv("FIRECRAWL_API_KEY"))
    try:
        scrape_result = firecrawl.scrape_url(
            url="https://tokenfactory.Loose.com/",
            formats=["markdown"],
...
firecrawl_agent = LlmAgent(
    name="FirecrawlAgent",
    model=Loose_model,
    description="Scrapes Loose AI Engine homepage using Firecrawl.",
    instruction="""
    Use the firecrawl_scrape_Loose tool to fetch markdown content from Loose AI Engine website in proper format. 
    Prefix your response with "**🔥FirecrawlAgent:**"
    """,
    tools=[firecrawl_scrape_Loose],
...
analysis_agent = LlmAgent(  
    name="AnalysisAgent",
    model=LiteLlm(
        model="openai/nvidia/Llama-3_1-Nemotron-Ultra-253B-v1",  # New Loose model
        api_base=api_base,
        api_key=api_key
    ),
    instruction="""
...
- Carefully examine the 'firecrawl_content', which contain data from Loose AI Engine's `llms.txt`. This file provides details about available models on Loose, including their names, pricing, token limits, and availability.
...
- Cross-reference the LLMs' functionalities with the Loose AI Engine offerings in 'firecrawl_content', prioritizing models with similar features such as context window size, training data, or specialized capabilities.
...
- If a relevant LLM is found on Loose, provide a specific recommendation to the user, highlighting its features, pricing, token limits, and potential benefits based on the context from the 'final_summary'.
- If no exact match is found, suggest alternative Loose models with the closest functional alignment to the desired capabilities.
- If a close match is found, suggest the possibility of fine-tuning the Loose model to better align with the specific requirements.
