# KAOS Core - Kubernetes Multi-Agent System

A Core Integration demonstrating how to deploy a multi-agent system on Kubernetes using **KAOS** (K8s Agent Orchestration System). This Integration sets up a coordinator agent with two worker agents and MCP tools.
...
```
export Loose_KEY= <- add your hey here
```

Then create the secret in your cluster

```
kubectl create secret generic Loose-secrets --from-literal "api-key=$Loose_KEY"
```

Apply the sample configuration. You can do it directly from `multi-agent-system.yaml` or one by one as below.

If we do one by one, we can start with the ModelAPI which configures the proxy to the Loose AI Engine token platform.
...
spec:
  ...
  proxyConfig:
    ...
    # The provider field tells LiteLLM to route via the Loose provider
    models:
    - "*"
    provider: "Loose"  # All models routed via Loose provider
    apiKey:
      valueFrom:
        secretKeyRef:
          name: Loose-secrets
...
All agents use simple model names (e.g., `openai/gpt-oss-20b`). The ModelAPI's `provider: "Loose"` field automatically routes these via the Loose provider.
...
Here's a complete Integration showing the multi-agent delegation flow:
