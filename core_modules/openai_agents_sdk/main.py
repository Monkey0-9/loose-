...
if not api_key:
    raise ValueError("Loose_API_KEY is not set in the environment variables")
base_url = os.getenv("Integration_BASE_URL", "https://api.tokenfactory.Loose.com/v1")
model_name = os.getenv("Integration_MODEL_NAME", "meta-llama/Meta-Llama-3.1-8B-Instruct")
...
def send_email(to:str, subject:str, body:str):
    ...
    params = {
        "from": "hello@looseai.dev",  # Replace with your verified sender email
        "to": [to],
        "subject": subject,
        "html": body,
    }
...
async def main():
    """
    Runs an Integration agent that sends an email using a haiku response style.
