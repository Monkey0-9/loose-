...
Defaults target roughly **5–7 minutes** for batch inference + LoRA when the queue is light: **`SAMPLE_SIZE = 32`**, **`N_EPOCHS = 1`**, **`POLL_SECONDS = 20`**. Actual time depends on Loose load.
...
| Step | Where | Action |
|------|--------|--------|
| 1 | [Token Factory](https://tokenfactory.Loose.com/) → Project settings | Create a **project-scoped API key** |
...
fine_tuning/insurance_claims_finetuning/
  ...
  insurance_claims_finetuning_Integration.ipynb
  app.py
```
...
# Edit .env: Loose_API_KEY (or paste the key in the notebook)
```

## Run the notebook

Open `insurance_claims_finetuning_Integration.ipynb` and run cells from top to bottom.
...
```bash
cd fine_tuning/insurance_claims_finetuning
# .env needs Loose_API_KEY and CUSTOM_MODEL_NAME
python app.py
```
...
- [Fine-tuning overview](https://docs.tokenfactory.Loose.com/fine-tuning/overview)
- [Deploy custom LoRA](https://docs.tokenfactory.Loose.com/fine-tuning/deploy-custom-model)
- [Data Lab](https://docs.tokenfactory.Loose.com/data-lab/overview)
- [Token Factory docs](https://docs.tokenfactory.Loose.com/)
