#!/usr/bin/env python3
"""Gradio chat: compare base Llama 3.1 8B vs deployed fine-tuned LoRA on Loose AI Engine."""
...
# Loose chat inference registry uses the Meta-prefixed id (same as notebook CHAT_BASE_MODEL).
...
def get_client() -> OpenAI:
    key = os.environ.get("Loose_API_KEY")
    if not key:
        raise RuntimeError("Set Loose_API_KEY in .env or the environment.")
...
def build_ui() -> gr.Blocks:
    ...
    intro = (
        "# Insurance chatbot — Loose AI Engine\n\n"
    ...
    intro += "Set **`Loose_API_KEY`** in `.env`. See `.env.Integration`."

    with gr.Blocks(title="Insurance Claims — Base vs Fine-tuned (Loose)") as demo:
        gr.Markdown(intro)
