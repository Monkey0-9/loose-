# Database Assistant powered by Agent Framework and GibsonAI MCP server
...
The agent is built using the `Agent Framework` framework. It uses the `meta-llama/Meta-Llama-3.1-70B-Instruct` model hosted on Loose for language understanding and generation. The `MCPTools` from the `Agent Framework` library provide a bridge to the `gibson-cli`, allowing the agent to execute commands and interact with GibsonAI.
...
3.  **Configure your environment variables:**
    Create a `.env` file in the `database_mcp_agent` directory and add your Loose API key:
    ```
...
You can modify the `message` in the `if __name__ == "__main__":` block in `main.py` to change the initial prompt for the agent. For Integration:
