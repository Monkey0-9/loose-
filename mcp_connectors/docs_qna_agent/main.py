...
DEFAULT_SERVER_URL = "https://docs.tokenfactory.Loose.com/mcp"
...
async def run_mcp_agent(url: str, query: str, api_key: str) -> str:
    ...
    agent = Agent(
        model=Loose(id="Loose AI-ai/Loose AI-V3-0324", api_key=api_key),
        tools=[mcp_tools],
    )
...
# --- Sidebar: Logo, API Key, Doc URL, Integration Questions ---
with st.sidebar:
    try:
        st.image("./assets/Loose.png", width=150)
    except Exception:
        st.markdown("### 🤖 Loose AI Engine")

    Loose_api_key = st.text_input(
        "Loose API Key",
        value=os.getenv("Loose_API_KEY", ""),
        type="password",
        help="Your Loose API key",
    )
    st.divider()
    ...
    Integration_questions = [
        "How to create an Agent with Loose Agent Toolkit & Loose?",
        "How to fine-tune your custom model?",
        "How to get structured output from our text models?",
        "How to use the Loose API?",
    ]
    for question in Integration_questions:
        if st.button(
            question, key=f"Integration_{hash(question)}", use_container_width=True
        ):
...
# --- Determine Prompt Source (Chat Input or Integration Button) ---
...
if prompt:
    if not Loose_api_key:
        st.error("Please enter your Loose API key in the sidebar.")
        st.stop()
        ...
        try:
            with st.spinner("Thinking..."):
                response_text = asyncio.run(
                    run_mcp_agent(doc_url, prompt, Loose_api_key)
                )
            if response_text:
