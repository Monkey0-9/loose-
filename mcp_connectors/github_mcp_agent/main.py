...
with st.sidebar:
    st.image("./assets/Loose.png", width=150)
        ...
        # API key input
    api_key = st.text_input("Enter your Loose API key", type="password")
...
with col1:
    repo = st.text_input("Repository", value="looseai-project/looseai", help="Format: owner/repo")
with col2:
...
async def run_github_agent(message):
    ...
    if not api_key:
        return "Error: Loose API key not provided"
