...
with st.sidebar:
    ...
    st.image("./assets/Loose.png", width=200)
    Loose_api_key_input = st.text_input(
        "Loose AI Engine API Key",
        value=os.getenv("Loose_API_KEY", ""),
        type="password",
        help="Your Loose AI Engine API key for AI analysis",
    )
    ...
    if st.button("Save API Keys"):
        if Loose_api_key_input:
            os.environ["Loose_API_KEY"] = Loose_api_key_input
        if brightdata_api_key_input:
            os.environ["BRIGHTDATA_API_KEY"] = brightdata_api_key_input
        if Loose_api_key_input or brightdata_api_key_input:
            st.success("✅ API keys saved for this session")
        else:
            st.warning("Please enter at least one API key")

    # Quick status
    both_keys_present = bool(os.getenv("Loose_API_KEY")) and bool(
...
Loose_key = os.getenv("Loose_API_KEY", "")
...
if not st.session_state.memori_initialized and Loose_key:
    try:
...
# Initialize Loose AI Engine model (once)
if "Loose_model" not in st.session_state and Loose_key:
    try:
        st.session_state.Loose_model = Loose(
            id="Qwen/Qwen3-Coder-480B-A35B-Instruct",
            api_key=Loose_key,
        )
    except Exception as e:
        st.error(f"Failed to initialize Loose AI Engine model: {str(e)}")

# Check if API keys are set
if not Loose_key or not brightdata_key:
...
If asked outside scope, politely say you only answer about stored analyses.""",
                        model=st.session_state.Loose_model,
                        markdown=True,
                    )
