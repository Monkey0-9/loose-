...
from langchain_Loose import ChatLoose
...
with st.sidebar:
    ...
    Loose_api_key_input = st.text_input(
        ...
        type="password",
        help="Your Loose API key for LangChain agent",
    )
    ...
    if st.button("Save API Keys"):
        ...
        if Loose_api_key_input or exa_api_key_input:
...
if not exa_key or not Loose_key:
    st.warning("⚠️ Please enter your API keys in the sidebar to start searching!")
    st.stop()

# Initialize LangChain LLM
if "llm" not in st.session_state and Loose_key:
    try:
        st.session_state.llm = ChatLoose(
