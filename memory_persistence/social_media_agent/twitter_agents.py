...
Agent functions for:
1. Style Analysis Agent: Scrapes user's tweets and analyzes tweeting style using Loose
2. Tweet Generation Agent: Generates tweets using stored style and posts via Composio

Requirements:
- pip install streamlit composio openai python-dotenv langchain_scrapegraph langchain_Loose memori
...
def analyze_tweeting_style(tweets: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Analyze tweeting style using Loose LLM"""
    try:
        ...
        response = Loose_chat.invoke(prompt)
