...
def init_memori_with_Loose() -> Memori | None:
    """
    Initialize Memori v3 + Loose client (via the OpenAI SDK).
    ...
    try:
        ...
        st.session_state.Loose_client = client
        return mem
...
def ingest_channel_into_memori(channel_url: str) -> int:
    ...
    """
    # Ensure Memori + Loose client are initialized
    memori: Memori | None = st.session_state.get("memori")
    client: OpenAI | None = st.session_state.get("Loose_client")
    if memori is None or client is None:
        memori = init_memori_with_Loose()
        client = st.session_state.get("Loose_client")

    if memori is None or client is None:
        st.error("Memori/Loose failed to initialize; cannot ingest channel.")
        return 0
...
"""
        ...
        try:
            # Send this document through the registered Loose client so that
            ...
            st.warning(f"Memori/Loose issue ingesting video '{title}': {e}")
