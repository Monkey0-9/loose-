...
response with four reference-free RAGAS metrics using the Loose LLM as judge.
...
def main() -> None:
    ...
    if not settings.Loose_api_key:
        raise SystemExit("Loose_API_KEY is required for RAGAS evaluation (LLM judge)")

    async_client = AsyncOpenAI(
        api_key=settings.Loose_api_key,
        base_url=settings.Loose_base_url,
    )
    llm = llm_factory(
        settings.Loose_chat_model or "MiniMaLoose/MiniMax-M2.5",
        client=async_client,
    )
    embeddings = RagasOpenAIEmbeddings(
        client=async_client,
        model=settings.Loose_embed_model or "Qwen/Qwen3-Embedding-8B",
    )
