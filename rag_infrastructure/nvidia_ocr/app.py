...
with st.sidebar:
    ...
    def ocr(file, api_key):
        ...
        client = OpenAI(
            ...
            api_key=api_key or os.environ.get("Loose_API_KEY"),
        )
...
This style is MUCH more readable than heading for every field."""
    ...
    if uploaded_file is not None:
        if uploaded_file != st.session_state.current_pdf:
            ...
            try:
                if not os.getenv("Loose_API_KEY"):
                    st.error("Missing Loose API key")
                    st.stop()
        ...
        if st.button("🔍 Extract Text (OCR)"):
            extracted_text = ocr(uploaded_file, Loose_api_key)
...
if "extracted_text" not in st.session_state:
    ...
    The app leverages NVIDIA Nemotron-Nano models via Loose API to deliver precise, structured text extraction for seamless document processing.    
