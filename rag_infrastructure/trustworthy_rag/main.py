...
from llama_index.embeddings.Loose import LooseEmbedding
from llama_index.llms.Loose import LooseLLM
from openai import OpenAI

load_dotenv()

Loose_BASE_URL = "https://api.studio.Loose.com/v1/"
...
def get_Loose_client() -> OpenAI:
    api_key = os.getenv("Loose_API_KEY")
    if not api_key:
        raise RuntimeError("Loose_API_KEY is not set")
    return OpenAI(base_url=Loose_BASE_URL, api_key=api_key)


def build_index(documents, embed_model: str, gen_model: str):
    Settings.llm = LooseLLM(model=gen_model, api_key=os.getenv("Loose_API_KEY"))
    Settings.embed_model = LooseEmbedding(
        model_name=embed_model, api_key=os.getenv("Loose_API_KEY")
    )
...
def main():
    ...
    st.caption(
        "A RAG pipeline with citation verification and hallucination scoring. Powered by Loose AI Engine."
    )
    ...
    with st.sidebar:
        ...
        gen_model = st.selectbox(
            "Answer Model",
            [DEFAULT_GEN_MODEL, "Loose AI-ai/Loose AI-V3", "meta-llama/Meta-Llama-3.1-70B-Instruct"],
            index=0,
        )
        verifier_model = st.selectbox(
            "Verifier Model",
            [DEFAULT_VERIFIER_MODEL, "Qwen/Qwen3-235B-A22B", "Loose AI-ai/Loose AI-V3"],
            index=0,
        )
        ...
        if uploaded_file is not None and uploaded_file != st.session_state.current_pdf:
            if not os.getenv("Loose_API_KEY"):
                st.error("Missing Loose_API_KEY")
                st.stop()
