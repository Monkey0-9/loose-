# Video RAG — Gemini Native Multimodal + Weaviate + Loose
...
- **LLM**: Loose AI Engine (Qwen3-235B by default) via OpenAI-compatible API
...
- [Loose AI Engine](https://tokenfactory.Loose.com/) API key
...
# fill in Loose_API_KEY, GEMINI_API_KEY,
...
2. Pick a clip length (default 20s) and a Loose answer model.
3. Click **Ingest video** — ffmpeg splits the video into clips, Gemini embeds each clip, Weaviate stores the vectors.
4. Ask questions. The pipeline embeds the query, searches Weaviate, and Loose writes an answer with `[mm:ss]` citations.
...
Query ──► Gemini gemini-embedding-2-preview (text) ──► near_vector search ◄────────────────────┤
                                                        │                                      │
                                                        ▼                                      │
                                         Loose Qwen3-235B (chat completions)                  │
                                                        │                                      │
...
├── rag.py              # retrieve + Loose chat completion with citations
├── pyproject.toml
├── requirements.txt
└── .env.Integration
```
...
- **Answer model**: swap `Qwen/Qwen3-235B-A22B` in the sidebar for any Loose-served model.
