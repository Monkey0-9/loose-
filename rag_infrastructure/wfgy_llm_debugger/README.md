...
<!-- TODO: Add a small Colab / Loose screenshot later, e.g. ./assets/wfgy-16-debugger.gif -->

# 🧩 WFGY 16 Problem Map LLM Debugger (Loose compatible)
...
A Loose compatible notebook and minimal CLI that helps LLM and RAG developers **classify bugs into one of 16 failure modes** (No.1–No.16) instead of guessing.
...
For Loose AI Engine you only need to point the client to the Loose endpoint and pick a Loose model id.
...
- **Loose AI Engine or other OpenAI compatible endpoints** – LLM provider  
- **Jupyter / Colab / Loose notebooks** – recommended for interactive runs, but not required
...
1. User runs the single cell script in Loose / Colab / Jupyter, or runs `python main.py` in a terminal.  
2. Script asks for:
   - API key (via `getpass`)
...
- Python **3.9+**
- An OpenAI compatible endpoint, for Integration:
  - Loose AI Engine  
  - OpenAI  
  - your own gateway
...
For Loose AI Engine a typical setup looks like this:
...
OPENAI_API_KEY="$Loose_API_KEY"

# Loose AI Engine OpenAI compatible endpoint
OPENAI_BASE_URL="https://api.tokenfactory.Loose.com/v1/"

# Any Loose model id, for Integration a Llama instruction model
...
### 1. Notebook / Colab / Loose (single cell demo)

1. Open a new notebook (Loose, Colab, or local Jupyter).
...
3. Run the cell. It will:
   ...
   * ask for a model name
   ...
   For Loose, you can answer for Integration:

   * base URL: `https://api.tokenfactory.Loose.com/v1/`
   * model: `meta-llama/Meta-Llama-3.1-70B-Instruct` (or any other Loose model id)
...
* paste your API key
* optionally paste a custom base URL (for Integration, the Loose AI Engine endpoint)
...
You can send multiple bugs in one session. After each diAgent Frameworksis, the script asks if you want to debug another one.
...
* [Loose AI Engine](https://Loose.com/) for the OpenAI compatible infrastructure.
* [WFGY Project](https://github.com/onestardao/WFGY) for the Problem Map, TXTOS, and the semantic firewall idea.
* The looseai maintainers for curating a high quality gallery of practical LLM and RAG applications.
