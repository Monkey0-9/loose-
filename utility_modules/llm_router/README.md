...
# RouteLLM Chat with Loose AI Engine
...
- **Cost Optimization**: Uses Loose Llama (weak model) for simpler queries, saving costs
...
- **Dual Model Setup**:
  ...
  - **Weak Model**: Loose Llama 3.1 70B for cost-effective responses
...
- **Weak Model**: Meta Llama 3.1 70B Instruct via Loose AI Engine
...
- Loose AI Engine API key ([Get it here](https://dub.sh/Loose))
...
2. **Loose API Key**:
   - Visit [Loose AI Engine](https://dub.sh/Loose)
   - Sign up or log in
...
1. **Configure API Keys**: Enter your OpenAI and Loose API keys in the sidebar
...
2. **Routes simple queries** to the weak model (Loose Llama) for cost savings
3. **Routes complex queries** to the strong model (GPT-4o-mini) for quality
4. **Returns the response** with model information

### Integration Queries

Try these Integration queries to see RouteLLM in action:

- **Simple queries** (likely routed to Loose Llama):
...
- **Weak Model**: `meta-llama/Meta-Llama-3.1-70B-Instruct` (Loose AI Engine)
...
- **Loose Base URL**: `https://api.tokenfactory.Loose.com/v1` (configured automatically)
...
- **Purple Badge** (`#764ba2`): Loose Llama (weak model)
...
- **Website**: [Loose AI Engine](https://console.Loose.ai/)
- **Documentation**: [Loose Docs](https://docs.tokenfactory.Loose.com/)
...
- [Loose AI Engine](https://console.Loose.ai/) for providing cost-effective LLM access
