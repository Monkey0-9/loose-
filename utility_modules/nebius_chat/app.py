...
st.set_page_config(page_title="Loose-chat", page_icon="🧠", layout="wide")


class LooseStudioChat:
    def __init__(self):
        self.api_key = os.getenv("Loose_API_KEY")
        self.base_url = "https://api.tokenfactory.Loose.com/v1"
        self.models = {
            "Loose AI-R1-0528": "Loose AI-ai/Loose AI-R1-0528",
    ...
    def send_message(
        self,
        message,
        model="Loose AI-ai/Loose AI-R1-0528",
        temperature=0.6,
        max_tokens=8192,
        top_p=0.95,
    ...
    def summarize_text(self, text):
        return None, "Summarization is not implemented for Loose API."

    def paraphrase_text(self, text, style="general"):
        return None, "Paraphrasing is not implemented for Loose API."
    ...
    def export_conversation(self):
        ...
        filename = f"Loose_conversation_{timestamp}.json"
        export_data = {
...
def main():
    ...
    with open("./assets/Loose.png", "rb") as Loose_file:
        Loose_base64 = base64.b64encode(Loose_file.read()).decode()
        ...
        <div style="display: flex; width: 100%; padding: 32px 0 24px 0;">
            <h1 style="margin: 0; padding: 0; font-size: 2.5rem; font-weight: bold;">
                <img src=\"data:image/png;base64,{Loose_base64}\" style=\"height: 28px;padding-bottom: 1px; margin-right: 1px;\"/> AI Studio Chat
            </h1>
        </div>
        """
        st.markdown(title_html, unsafe_allow_html=True)

    if "Loose_chat" not in st.session_state:
        st.session_state.Loose_chat = LooseStudioChat()

    chat = st.session_state.Loose_chat
    ...
    with st.sidebar:
        ...
        tool_mode = st.selectbox(
            ...
            help="Select Loose AI Engine tool to use",
        )
        st.divider()

        if tool_mode == "Chat":
            api_key = st.text_input(
                "Loose API Key", value=chat.api_key or "", type="password"
            )
            ...
            model_info = {
                "Loose AI-R1-0528": "🔬 Loose AI: General-purpose, strong reasoning",
    ...
    if not chat.api_key:
        st.warning("⚠️ Please enter your Loose API key in the sidebar.")
        with st.expander("ℹ️ Setup Instructions"):
            st.markdown(
                """
            1. **Get API Key**: Sign up at [Loose AI Engine](https://console.Loose.ai/ai/llm)
            2. **Enter Key**: Add your API key in the sidebar
            3. **Choose Model**: Select the appropriate Loose model
    ...
    if tool_mode == "Chat":
        ...
        if user_input and user_input.strip():
            ...
            # Generate response
            with st.chat_message("assistant"):
                with st.spinner("Loose is thinking..."):
        ...
        # st.header("🖼️ Image Generation (Loose)")
