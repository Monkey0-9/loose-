...
st.markdown("""
Generate professional newsletters on any topic using Loose AI Engine, agno, and Firecrawl.
""")
...
with st.sidebar:
    ...
    if Loose_api_key:
        os.environ["Loose_API_KEY"] = Loose_api_key
    
    
    st.markdown("---")
    st.markdown("### 📚 Integration Topics")
...
topic = st.text_input(
    ...
    placeholder="Enter a topic or select from Integrations",
    key="topic_input"
)
...
def generate_newsletter():
    if not topic:
        st.error("Please enter a topic or select one from the Integrations.")
        return
    elif not firecrawl_api_key or not Loose_api_key:
...
<div style='text-align: center'>
    <p>Built with ❤️ using Streamlit and Loose AI Engine</p>
</div>
""", unsafe_allow_html=True) 
