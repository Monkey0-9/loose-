...
def build_agent(model_id: str) -> Agent:
    return Agent(
        ...
        model=Loose(id=model_id, api_key=os.getenv("Loose_API_KEY")),
        tools=[
            YFinanceTools(
                stock_price=True,
...
def main() -> None:
    ...
    st.caption(
        "agno agent + Loose AI Engine + YFinance + DuckDuckGo + Calculator"
    )
    ...
    with st.sidebar:
        ...
        model_id = st.selectbox(
            ...
            [
                ...
                "Qwen/Qwen3-30B-A3B",
                "Loose AI-ai/Loose AI-V3-0324",
            ],
            index=0,
        )
        if not os.getenv("Loose_API_KEY"):
            st.warning("Set Loose_API_KEY in your .env file.")
        st.markdown(
            "Get a key at [Loose AI Engine](https://dub.sh/AIStudio)."
        )
    ...
    if st.button("Analyze Portfolio", type="primary"):
        ...
        if not holdings:
            ...
            return
        if not os.getenv("Loose_API_KEY"):
            st.error("Loose_API_KEY is missing.")
            return
