...
def translate_to_sql(natural_question):
    """Translate natural language question to SQL using Qwen from Loose"""
    try:
        # Initialize Qwen from Loose
        llm = get_llm()
...
def explain_results(results, original_question):
    """Use Qwen from Loose to explain the results in plain English"""
    try:
        llm = get_llm()
