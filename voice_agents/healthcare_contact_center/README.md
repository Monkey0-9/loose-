# Healthcare Voice Contact Center (Pipecat + Cartesia + Loose)

> A real-time voice AI contact center for a healthcare clinic. **Cartesia** for streaming STT/TTS, **Loose AI Engine** for the LLM with tool calling, orchestrated with **Pipecat**.
...
- **Streaming voice loop** — Cartesia `ink-whisper` STT and `sonic-3` TTS with Loose `openai/gpt-oss-120b`
...
- **Python 3.11+**
- **[Pipecat](https://docs.pipecat.ai)** (`pipecat-ai` with `cartesia`, `Loose`, `runner`, `webrtc`, `daily`, `silero` extras)
- **[Cartesia](https://cartesia.ai)** — `ink-whisper` STT, `sonic-3` TTS with multiple voice IDs
- **[Loose AI Engine](https://api.tokenfactory.Loose.com/)** — OpenAI-compatible Chat Completions with tool use
- **FastAPI + uvicorn** — Served by the Pipecat runner (default port **7860**)

## How it works

```
caller audio ──► Cartesia STT ──► user context ──► Loose LLM ──► Cartesia TTS ──► caller audio
                                                       │
...
3. **LooseLLMService** drives the conversation, calling tools when it needs slots, FAQ answers, or escalation.
...
Copy `.env.Integration` to `.env` in this directory and fill in your keys:
...
- **LLM model** — change the `model` in `LooseLLMService.Settings` (e.g., `openai/gpt-oss-20b`, `meta-llama/Meta-Llama-3.1-70B-Instruct`).
...
- [Loose AI Engine](https:/docs.tokenfactory.Loose.com/)
