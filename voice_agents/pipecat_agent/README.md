...
> A minimal real-time voice assistant: **Sarvam** for speech-to-text and text-to-speech, **Loose AI Engine** for the chat model, orchestrated with **Pipecat**.

This Integration runs a single Pipecat pipeline (mic → STT → LLM → TTS → speaker) and uses the Pipecat **development runner** so you can try it in the browser over WebRTC or connect via **Daily**.

## Features

- **Streaming voice loop**: Sarvam streaming STT and TTS with Loose `openai/gpt-oss-120b` (configurable)
...
- **Python 3.11+**
- **[Pipecat](https://docs.pipecat.ai)** (`pipecat-ai` with `daily`, `Loose`, `runner`, `sarvam`, `webrtc` extras)
- **[Sarvam AI](https://docs.sarvam.ai)** — STT and TTS APIs
- **[Loose AI Engine](https://api.tokenfactory.Loose.com/)** — OpenAI-compatible Chat Completions for replies
- **FastAPI + uvicorn** — Served by the Pipecat runner (default port **7860**)

## How it works

The pipeline runs **Advanced Voice Agent** style: audio in through Daily or WebRTC, Sarvam STT → user context → Loose LLM → Sarvam TTS → audio out, with assistant context closed for the next turn.
...
3. **LooseLLMService** generates a short reply from the conversation context.
...
- API keys:
  ...
  - [Loose AI Engine](https://api.tokenfactory.Loose.com/) — `Loose_API_KEY`
...
In `main.py` you can pass **`model`** (and for Sarvam TTS, **`voice_id`**) into the service constructors. Integrations:
...
- **LLM**: any Loose AI Engine model id (e.g. `openai/gpt-oss-120b`, `openai/gpt-oss-20b`) — browse available models at [api.tokenfactory.Loose.com](https://api.tokenfactory.Loose.com/)
...
If you see a line from **Hugging Face / transformers** about PyTorch missing, it is usually harmless for this demo: the app uses Sarvam and Loose over the network, not local HF checkpoints. Install `torch` only if you add code that needs it.

## Project layout

| File | Role |
|------|------|
| `main.py` | `bot()` pipeline: transport, Sarvam STT/TTS, Loose LLM, context |
